    ...
    def update_peripherals_status(self):
        if self.updating_peripherals_status and self.peripherals_thread is not None:
            ...
            if hasattr(self, '_peripheral_update_running') and self._peripheral_update_running:
                ...
                threading.Thread(target=self._update_peripherals_async, daemon=True).start()
        
        self.after(10000, self.update_peripherals_status)  # Update every 10 seconds
    
    def _update_peripherals_async(self):
        """Async peripheral status update"""
        try:
            self.after(0, self.update_filter_position)
            time.sleep(0.05)
            self.after(0, self.update_outlet_states)
            time.sleep(0.05)
            self.after(0, self.update_shutter)
            time.sleep(0.05)
            self.after(0, self.update_slit_position)
            time.sleep(0.05)
            self.after(0, self.update_halpha_qwp)
            time.sleep(0.05)
            self.after(0, self.update_pol_stage)
            time.sleep(0.05)
            self.after(0, self.update_zoom_stepper)
        except Exception as e:
            debug_logger.error(f"Peripherals update error: {e}")
        finally:
            self._peripheral_update_running = False
